...
import {
  TraineeCertificateDocument,
  type TraineeProgram,
  type TraineeRecipient,
  type TraineeTemplate,
} from "@/components/generator/TraineeCertificateDocument";
import { certificateFileBase, csvToRecipients, SAMPLE_CSV } from "@/lib/csv";
...
export interface TraineeCopy {
  ...
  generating: string;
  noRecipients: string;
  selectProgramFirst: string;
}

/** Capture width (px) for export — fixed so output is resolution-deterministic. */
const EXPORT_WIDTH = 1000;

type TabId = "data" | "program" | "template" | "preview" | "export";
const TABS: TabId[] = ["data", "program", "template", "preview", "export"];

function fileBase(recipient: TraineeRecipient): string {
  return certificateFileBase(recipient.credentialId);
}
...
const META_FILENAME = "masterfabric-academy-trainee-certificates.csv";

function buildMetadataCsv(
  recipients: TraineeRecipient[],
  ...
  const rows = recipients.map((r) => {
    ...
    return [
      ...
      template.instructorLinkedin
        ...
        : "",
      template.verifyUrl
        ? `${template.verifyUrl.replace(/\/$/, "")}/${certificateFileBase(r.credentialId)}`
        : "",
...
async function nodeToPng(node: HTMLElement): Promise<string> {
  ...
  return toPng(node, { pixelRatio: 2, cacheBust: true, backgroundColor: "#000000" });
}
...
/** Combines captured pages into a single multi-page PDF. */
function pagesToPdf(pages: CapturedPage[]): jsPDF {
...
export function TraineeCertificateGenerator({
  copy,
  initialPrograms,
  defaults,
}: {
  copy: TraineeCopy;
  initialPrograms: TraineeProgramInfo[];
  defaults: TraineeTemplate & { credentialPrefix: string; sampleName: string };
}) {
  const [credentialPrefix, setCredentialPrefix] = useState(defaults.credentialPrefix);
  const [template, setTemplate] = useState<TraineeTemplate>(defaults);
  const [recipients, setRecipients] = useState<TraineeRecipient[]>([
    { name: defaults.sampleName, credentialId: `${defaults.credentialPrefix}-0001` },
  ]);
  const [current, setCurrent] = useState(0);
  const [pasteText, setPasteText] = useState("");
  const [zipFormat, setZipFormat] = useState<"png" | "pdf">("pdf");
  const [busy, setBusy] = useState<{ done: number; total: number } | null>(null);
  const [activeTab, setActiveTab] = useState<TabId>("data");

  const [programs, setPrograms] = useState<TraineeProgramInfo[]>(initialPrograms);
  const [selectedSlug, setSelectedSlug] = useState<string>(
    initialPrograms[0]?.slug ?? "",
  );
  const [program, setProgram] = useState<TraineeProgram | null>(null);
  const [programState, setProgramState] = useState<"idle" | "loading" | "error">(
    "idle",
  );

  const exportRef = useRef<HTMLDivElement>(null);

  const setField = <K extends keyof TraineeTemplate>(
    key: K,
    value: TraineeTemplate[K],
  ...
  async function reloadPrograms() {
    ...
    try {
      const res = await fetch("/api/trainee-roadmap");
      const data = await res.json();
      if (!res.ok) throw new Error(data.error ?? "failed");
      setPrograms(data.programs ?? []);
      if (!selectedSlug && data.programs?.[0]) {
        setSelectedSlug(data.programs[0].slug);
      }
  ...
  // Load the roadmap whenever the selected program changes.
  useEffect(() => {
    if (!selectedSlug) return;
    let cancelled = false;
    setProgramState("loading");
    fetch(`/api/trainee-roadmap?program=${encodeURIComponent(selectedSlug)}`)
      .then(async (res) => {
        const data = await res.json();
        if (!res.ok) throw new Error(data.error ?? "failed");
        if (cancelled) return;
        const loaded = data.roadmap as TraineeProgram;
        setProgram(loaded);
        setProgramState("idle");
        // Keep the editable program line in sync with the selection.
        setTemplate((t) => ({
          ...t,
          program: defaultProgramLine(loaded.name),
        }));
      })
      .catch(() => {
        if (!cancelled) setProgramState("error");
      });
    return () => {
      cancelled = true;
    };
  ...
  function updateRecipient(index: number, patch: Partial<TraineeRecipient>) {
  ...
  /** Returns the two page nodes for the recipient at the given index. */
  function getPageNodes(index: number): HTMLElement[] {
    const root = exportRef.current;
    if (!root) return [];
    return Array.from(
      root.querySelectorAll<HTMLElement>(`[data-recipient="${index}"]`),
    );
  }

  async function capturePages(index: number): Promise<CapturedPage[]> {
    const nodes = getPageNodes(index);
  ...
  async function exportCurrent(format: "png" | "pdf") {
    ...
    try {
      const pages = await capturePages(safeCurrent);
      if (pages.length === 0) return;
      const base = fileBase(activeRecipient);
      if (format === "png") {
        pages.forEach((page, i) =>
          triggerDownload(page.dataUrl, `${base}-p${i + 1}.png`),
        );
      } else {
        const pdf = pagesToPdf(pages);
  ...
  function downloadMetadata() {
    if (!activeRecipient) return;
    const csv = buildMetadataCsv([activeRecipient], template, program);
  ...
  async function exportZip(format: "png" | "pdf") {
    ...
    const zip = new JSZip();
    setBusy({ done: 0, total: recipients.length });
    try {
      for (let i = 0; i < recipients.length; i++) {
        const r = recipients[i];
        const pages = await capturePages(i);
        const base = fileBase(r);
        if (format === "png") {
          pages.forEach((page, p) =>
            zip.file(`${base}-p${p + 1}.png`, page.dataUrl.split(",")[1], {
              base64: true,
            }),
          );
        } else {
        ...
        setBusy({ done: i + 1, total: recipients.length });
      }
      zip.file(META_FILENAME, buildMetadataCsv(recipients, template, program));
      const blob = await zip.generateAsync({ type: "blob" });
      const url = URL.createObjectURL(blob);
      triggerDownload(url, `trainee-certificates-${format}-${recipients.length}.zip`);
      setTimeout(() => URL.revokeObjectURL(url), 4000);
    } finally {
      setBusy(null);
    }
  ...
  const hasProgram = program !== null && program.rows.length > 0;
  ...
  return (
    <div className="mx-auto max-w-6xl px-6 py-16">
      ...
      <div className="mt-8 space-y-8">
        ...
        {activeTab === "program" && (
          <section className="border border-white/15 p-5">
            ...
            <p className="mt-2 text-xs text-white/45">{copy.programHint}</p>

            <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-end">
              <label className="block">
                <span className="mb-1 block font-mono text-[10px] uppercase tracking-widest text-white/40">
                  {copy.programSelectLabel}
                </span>
                <select
                  value={selectedSlug}
                  onChange={(e) => setSelectedSlug(e.target.value)}
                  className="w-full border border-white/20 bg-black px-3 py-2 text-xs text-white outline-none transition focus:border-white/60"
                >
                  {programs.length === 0 && (
                    <option value="">{copy.programLoading}</option>
                  )}
                  {programs.map((p) => (
            ...
            <div className="mt-4 text-xs">
              {programState === "loading" && (
                <p className="text-white/45">{copy.programLoading}</p>
              )}
              {programState === "error" && (
                <p className="text-red-300/80">{copy.programError}</p>
              )}
              {programState === "idle" && program && (
                <div className="border border-white/15 p-4">
                  ...
                  <p className="mt-1 font-mono text-[10px] uppercase tracking-widest text-white/40">
                    {program.rows.length} {copy.roadmapRows}
                  </p>
                  ...
                  <div className="mt-3 max-h-48 space-y-1 overflow-y-auto">
                    {program.rows.map((row, i) => (
                      <div
                        key={`${row.days}-${i}`}
                        className="flex gap-3 border-b border-dashed border-white/10 pb-1 text-[11px]"
                      >
                        <span className="w-16 shrink-0 font-mono text-white/45">
                          {row.days}
                        </span>
                        <span className="text-white/75">{row.focus}</span>
                      </div>
                    ))}
                  </div>
                </div>
        ...
        {activeTab === "template" && (
          <section className="border border-white/15 p-5">
            ...
            <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2">
              ...
              <div className="sm:col-span-2">
                <Field
                  label={copy.fields.scopeIntro}
                  value={template.scopeIntro}
                  onChange={(v) => setField("scopeIntro", v)}
                  textarea
                />
              </div>
              <Field
        ...
        {activeTab === "preview" && (
          <section className="border border-white/15 p-5">
            ...
            {!hasProgram && (
              <p className="mt-3 font-mono text-[10px] uppercase tracking-widest text-amber-300/70">
                {copy.selectProgramFirst}
              </p>
            )}
      ...
      {/* Offscreen fixed-width render targets — two pages per recipient. */}
      <div
        ref={exportRef}
        aria-hidden
      ...
      >
        {recipients.map((r, i) => (
          <div key={i}>
            <div data-recipient={i} data-cert-page="1" style={{ width: EXPORT_WIDTH }}>
              <TraineeCertificateDocument
                template={template}
                recipient={r}
                program={program}
            ...
            <div data-recipient={i} data-cert-page="2" style={{ width: EXPORT_WIDTH }}>
              <TraineeCertificateDocument
                template={template}
                recipient={r}
                program={program}
...
function defaultProgramLine(name: string): string {
  return `${name} — 100 Days Trainee Program`;
}
